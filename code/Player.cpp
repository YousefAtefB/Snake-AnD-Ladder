...
void Player::Move(Grid* pGrid, int diceNumber)
{
			...
			if (x == 1)
			{
				...
				if (PP->Available(S))
				{
					...
					if (x == 1)
					{
						...
						if (PP->IceF(PN, pGrid))
						{
							pOut->PrintMessage("Ice activated,Click any where to continue...");
							pIn->GetCellClicked();
						}
					}
					else if (x == 2)
					{
						...
						{
							pOut->PrintMessage("Fire activated,Click any where to continue...");
					...
					else if (x == 3)
					{
						...
						{
							pOut->PrintMessage("Poison activated,Click any where to continue...");
						}
					}
					else if (x == 4)
					{
						...
						{
							pOut->PrintMessage("Lightning activated,Click any where to continue...");
	...
	else
	{
		if (Prevent == 0)
		{
			...
			if (pCell->GetCellPosition().GetCellNum()+diceNumber >= NumHorizontalCells * NumVerticalCells)
			{
				pGrid->UpdatePlayerCell(this, CellPosition(NumHorizontalCells* NumVerticalCells));
				pGrid->SetEndGame(true);
				pGrid->PrintErrorMessage("Congratulation you Won the game,Click any where to continue");
				return;
			}
			else
			{
